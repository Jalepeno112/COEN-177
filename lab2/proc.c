 ...
 *
 *
 * The kernel uses a round-robin priority queue for its scheduling.  
 */
   ...
   * Go through each queue from greatest priority to least, 
   * if the queue's head is not NULL, then we take that process and run it.

   * NR_SCHED_QUEUES is 16.  This is equal to the minimum priority + 1
   */
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
    ...
    * The maximum queue for a user process is at most 7.  So DO NOT randomly schedule if the priority is less than 7.
    */
    if (q >= 7) {
	/*declare variables*/
	u64_t time;
	...
	/* First decide if we want to override the current process.
  	 * Want the probablity to go up as we go down in priority.
	 */
	 if (rand%100 < (100/(NR_SCHED_QUEUES+1))*q) {
		/*the queue that we jump to is the lower half of random % the number of priority queues+1*/
		int random_q = rand%NR_SCHED_QUEUES;
		TRACE(VF_PICKPROC, printf("randomly picking new process! Jumping to queue %d\n",random_q););
		/*check to make sure that the priority queue we want to jump to has a process in it*/
		if (rdy_head[random_q]) {
			rp = rdy_head[random_q];
		}
	}
   }
	...
	/*TRACE is a debug tool.  Does not do anything for picking the process*/
